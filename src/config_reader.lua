--[[
  ...
  Kevin Lynx
  1.30.2011
  Save the config as UTF8 encode.
--]]
require("LuaXml")

CONFIG = {}
CONFIG_TAGS = { "loginname", "pcname", "nickname", "groupname", "macaddress", "savedir" }
...
function config_savedir()
    return CONFIG[CONFIG_TAGS[6]]
end
...
function config_load()
    ...
    for i,v in ipairs(CONFIG_TAGS) do
        ...
        if t == nil then
            ...
            CONFIG[v] = t.value --env_s_g2u(t.value)
...
function config_loadgroups(t)
    ...
    while t[i] ~= nil do
        ...
        group[name] = tag[name] --env_s_g2u(tag[name])
