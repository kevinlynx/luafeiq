...
function handle_recv_data(udp, data, ip, port)
	...
	local t = tonumber(msg_read_type(fullheader))
	local arg = mh_arg(udp, ip, port, fullheader, feiqheader, body, t)
	mh_dispatch(t, arg)
end
...
function mh_handle_group(arg)
	...
	local decrypt_body = ""
    if pickmask(arg.cmd, OPT_ENCRYPT) ~= 0 then
        decrypt_body = msg_decrypt_body(arg.body, bodys, mac)
    else
        decrypt_body = arg.body 
    end
    logi("-------------------------Group Message------------------------------------------")
    ...
    if groupnum == nil then 
        logw(string.format("invalid group msg: %s", decrypt_body))
        return
    end
...
function mh_handle_entry_group(arg)
	...
	local groupnum = msg_read_groupnum(decrypt_body)
    if groupnum == nil then 
        logw(string.format("invalid group entry msg: %s", decrypt_body))
        return
    end
    groupnum = check_string(groupnum)
