...
function branch_find(tree, name, kind, start)
    local i = 1
    local ret = false
    if start ~= nil then i = start end
    while true do 
        local namei = "name" .. i
        local kindi = "kind" .. i
...
function branch_findleaf(tree, bname, lname)
    local bid = branch_find(tree, bname, "BRANCH")
    if bid == 0 then return 0 end
    local lid = branch_find(tree, lname, "LEAF", bid+1)
    return lid
end
